...
namespace Ilumisoft.RadarSystem
{
    ...
    public class Radar : MonoBehaviour
    {
        ...
        private bool TryGetIconLocation(LocatableComponent locatable, out Vector2 iconLocation)
        {
            ...
            // Получаем размер радара (это полный размер квадрата, а не половина)
            float radarSize = GetRadarUISize();

            // Масштабируем координаты иконки с учетом полного размера радара
            var scale = radarSize / Range;
            iconLocation *= scale;

            // Ограничиваем иконки по краям квадрата
            // Теперь иконки должны быть по краям всего UI
            if (iconLocation.x > radarSize) iconLocation.x = radarSize;
            if (iconLocation.x < -radarSize) iconLocation.x = -radarSize;
            if (iconLocation.y > radarSize) iconLocation.y = radarSize;
            if (iconLocation.y < -radarSize) iconLocation.y = -radarSize;

            // Применяем поворот, если включено
            if (ApplyRotation)
            {
                // Получаем вектор направления вперед игрока, проекцированный на плоскость XZ
                var playerForwardDirectionXZ = Vector3.ProjectOnPlane(Player.transform.forward, Vector3.up);

                // Создаем вращение на основе этого направления
                var rotation = Quaternion.LookRotation(playerForwardDirectionXZ);

                // Отражаем вращение по оси Y
                var euler = rotation.eulerAngles;
                euler.y = -euler.y;
                rotation.eulerAngles = euler;

                // Применяем вращение к координатам иконки в 3D пространстве
                var rotatedIconLocation = rotation * new Vector3(iconLocation.x, 0.0f, iconLocation.y);

                // Переводим результат в 2D
                iconLocation = new Vector2(rotatedIconLocation.x, rotatedIconLocation.z);
            }

            // Если точка в пределах радара или она должна быть зафиксирована
            if (iconLocation.sqrMagnitude < radarSize * radarSize || locatable.ClampOnRadar)
            {
                // Ограничиваем точку в пределах квадрата
                iconLocation = new Vector2(Mathf.Clamp(iconLocation.x, -radarSize, radarSize),
                                           Mathf.Clamp(iconLocation.y, -radarSize, radarSize));
                return true;
            }
